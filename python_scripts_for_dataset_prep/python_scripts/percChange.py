...
with open(f'changePerc/changePercThresholdArousal.csv', 'w') as file:
    file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    file_writer.writerow(['Video', 'User', 'Threshold', 'Number of changes'])

with open(f'changePerc/changePercThresholdValence.csv', 'w') as file:
    file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    file_writer.writerow(['Video', 'User', 'Threshold', 'Number of changes'])

with open(f'changePerc/changePercThreshold.csv', 'w') as file:
    file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    file_writer.writerow(['Video', 'User', 'Threshold', 'Number of changes'])
...
for video in videos:
    for threshold in thresholds:
        ...
        numberValenceDifference = 0
        numberArousalDifference = 0
        numberScoreDifference = 0
        ...
        for i in range(len(pointsToProbePerSubject)):
            ...
            for j in range(len(pointsToProbePerSubject[i])):
                if(pointsToProbePerSubject[i][j] > threshold):
                    numberArousalDifference += 1
                    numberValenceDifference += 1
                    numtemp2 += 1
                    numtemp1 += 1
                    numtemp3 += 1
                    numberScoreDifference += 1
                total += 1
        ...
        for i in range(len(vid2_windows_5000)):
            with open(f'changePerc/changePercThresholdArousal.csv', 'a') as file:
                file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                file_writer.writerow([video, subjectsOrder[i], threshold, numberArousalDifferencePerSubject[i]])

        for i in range(len(vid2_windows_5000)):
            with open(f'changePerc/changePercThresholdValence.csv', 'a') as file:
                file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                file_writer.writerow([video, subjectsOrder[i], threshold, numberValenceDifferencePerSubject[i]])
        
        for i in range(len(vid2_windows_5000)):
            with open(f'changePerc/changePercThreshold.csv', 'a') as file:
                file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
