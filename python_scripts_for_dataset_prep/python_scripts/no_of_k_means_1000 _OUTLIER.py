...
directory = "1000/scores_" + str(video)
...
vid2_windows_1000_for_kmeans = []

#for i in range(1):
for i in range(len(vid2_windows_1000)):
    scores = vid2_windows_1000[i]['Score'].values
    ...
    for j in range(len(vid2_windows_1000[i]['Score'].values)):
        for k in range(len(outliers)):
            if(round(vid2_windows_1000[i]['Score'].values[j], 4) == round(outliers[k], 4)):
                val = vid2_windows_1000[i][round(vid2_windows_1000[i]['Score'],4) == round(outliers[k],4)]
                idxl = val.index.tolist()
                idx = idxl[0]
                idx_drop.append(idx)
    for j in range(len(vid2_windows_1000[i]['Score'].values)):
        for k in range(len(valid_outliers)):
            if(round(vid2_windows_1000[i]['Score'].values[j], 4) == round(valid_outliers[k], 4)):
                val = vid2_windows_1000[i][round(vid2_windows_1000[i]['Score'],4) == round(valid_outliers[k],4)]
                idxl = val.index.tolist()
                idx = idxl[0]
                idx_drop_kmeans.append(idx)
    vid2_windows_1000[i] = vid2_windows_1000[i].drop(idx_drop)
    vid2_windows_1000_for_kmeans.append(vid2_windows_1000[i].drop(idx_drop_kmeans))
...
#for i in range(1):
for i in range(len(vid2_windows_1000_for_kmeans)):
    X = vid2_windows_1000_for_kmeans[i]['Score'].values
    kmeans = KMeans(n_clusters=8).fit(X.reshape(-1,1))
    centers = kmeans.cluster_centers_
    choosen_centroid = np.max(centers)
...
for i in range(len(vid2_windows_1000)):
    ...
    for j in range(len(pointsToProbePerSubject[i])):
        df = vid2_windows_1000[i][vid2_windows_1000[i]['Score'] == pointsToProbePerSubject[i][j]]
...
with open('final1000.txt', 'w') as f:
