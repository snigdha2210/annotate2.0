...
for video in range(1,2):
    ...
    allTimeAnnoSmoothened = []
    allValenceSmoothened = []
    allArousalSmoothened = []
    chunksize = 100
    for i in range(len(subjects_annotation)):                                                                               
        temp1 = []
        temp2 = []
        temp3 = []
        for j in range(0, len(allTimeAnno[i]), chunksize):
            temp1.append(np.average(allTimeAnno[i][j : j+chunksize]))
        for j in range(0, len(allValence[i]), chunksize):
            temp2.append(np.average(allValence[i][j : j+chunksize]))
        for j in range(0, len(allArousal[i]), chunksize):
            temp3.append(np.average(allArousal[i][j : j+chunksize]))
        allTimeAnnoSmoothened.append(temp1)
        allValenceSmoothened.append(temp2)
        allArousalSmoothened.append(temp3)
    ...
    #for i in range(1):
        ...
        plt.tight_layout()
        plt.savefig(f"graphs_probed_smoothened_100/valence/video_{video}sub_{subjectsOrder[i]}.png")
        plt.close()
        # plt.show()
    for i in range(len(subjects_annotation)):
        plt.plot(allTimeAnnoSmoothened[i], allArousalSmoothened[i], label="continuous arousal")
        plt.plot(probedTimeNew[i], probedArousalNew[i], 'x', markevery=range(len(probedArousal[i]) - 1), linestyle='--', label='probed arousal')
        plt.xlabel("Time (in ms.)", fontsize=17)
        plt.ylabel("Arousal", fontsize=17)
        plt.grid(axis = 'y', color = "grey", linewidth = "1.4", linestyle = "--")
        plt.legend(fontsize=15)
        plt.tight_layout()
        plt.savefig(f"graphs_probed_smoothened_100/arousal/video_{video}sub_{subjectsOrder[i]}.png")
        plt.close()
        # plt.show()
